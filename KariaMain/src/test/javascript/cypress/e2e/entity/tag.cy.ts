...
describe('Tag e2e test', () => {
  ...
  const tagSample = { name: 'every blah gadzooks' };
  ...
  beforeEach(() => {
    cy.intercept('GET', '/services/property/api/tags+(?*|)').as('entitiesRequest');
    cy.intercept('POST', '/services/property/api/tags').as('postEntityRequest');
    cy.intercept('DELETE', '/services/property/api/tags/*').as('deleteEntityRequest');
  });
  ...
  describe('new Tag page', () => {
    ...
    it('should create an instance of Tag', () => {
      cy.get(`[data-cy="name"]`).type('er obtrude');
      cy.get(`[data-cy="name"]`).should('have.value', 'er obtrude');
